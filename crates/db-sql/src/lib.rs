...
pub fn bind(sql: &str, database: &Database) -> LogicalPlan {
    ...
    let table_name = bind_from_clause(&select.from);

    let table = database
        .get_table(&table_name)
        .unwrap_or_else(|| panic!("table not found: {table_name}"));

    let column_indices = bind_projection(&select.projection, table.schema());
